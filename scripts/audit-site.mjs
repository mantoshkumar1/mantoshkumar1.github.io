...
const pages = ["index.html", "projects/index.html", "insights/index.html", "insights/engineering-philosophy.html", "insights/why-does-this-still-require-me.html", "insights/release-reports-as-operational-history.html", "insights/complexity-changes-address.html", "insights/blockchain-without-a-master-branch.html", "insights/ownership-before-escalation.html", "insights/message-bus-between-ai-agents.html", "insights/make-the-banks-deterministic.html", "insights/tests-are-the-cheapest-check.html", "experience/index.html", "resume/index.html", "contact/index.html", "newsletter/index.html", "accessibility/index.html", "projects/engineering-knowledge-system.html", "projects/photosahi.html", "projects/workflow-automation-toolkit.html", "projects/gtt-price-calculator.html", "projects/validation-platform-optical-networking.html", "projects/legacy-validation-framework-migration.html", "projects/dogbuild.html"];
const requirements = [
...
if (!/class=["'][^"']*insight-discovery-actions[^"']*["'][^>]*>[\s\S]*href=["']insights\/["'][^>]*>View all insights[\s\S]*href=["']newsletter\/["'][^>]*>Join newsletter/i.test(homeHtml)) { console.error("homepage: closing insight actions need concise, visible labels"); failures += 1; }
...
for (const [page, html, expectedCards] of [["index.html", homeHtml, 3], ["projects/index.html", projectsHtml, 7]]) {
...
for (const page of pages.filter((entry) => entry.startsWith("projects/") && entry !== "projects/index.html")) {
  ...
  if (decisionParagraphs.length < 3 || decisionParagraphs.length > 6) { console.error(`${page}: Engineering Decisions must contain 3-6 decisions`); failures += 1; }
  if (decisionParagraphs.some((paragraph) => (paragraph.match(/[.!?](?:\s|$)/g) || []).length < 3)) { console.error(`${page}: every engineering decision must explain the choice, rationale, and trade-off concisely`); failures += 1; }
  const demonstrates = /<section\b[^>]*class=["'][^"']*project-demonstrates[^"']*["'][^>]*>([\s\S]*?)<\/section>/i.exec(html)?.[1] || "";
  if (!/<h2[^>]*>What This Project Demonstrates<\/h2>/i.test(demonstrates)) { console.error(`${page}: project needs a What This Project Demonstrates section`); failures += 1; }

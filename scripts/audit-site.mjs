...
if (!/class=["'][^"']*insight-discovery-actions[^"']*["'][^>]*>[\s\S]*href=["']insights\/["'][^>]*>Read insights[\s\S]*href=["']newsletter\/["'][^>]*aria-label=["']Join the newsletter for new insights by email["'][^>]*>Join newsletter/i.test(homeHtml)) { console.error("homepage: closing insight actions need concise, explicit labels"); failures += 1; }
...
if (!/Ranked in the top 1% in India’s GATE Computer Science &amp; IT examination/.test(resumeHtml) || !/href=["']\.\.\/experience\/#capabilities["'][^>]*>See how I help engineering teams/i.test(resumeHtml)) { console.error("resume: verified highlights or capabilities bridge is missing"); failures += 1; }

...
`related_topics`; the model does not invent them. Follow-up questions use short, question-aware templates that move naturally from the current topic toward ownership, evidence, outcomes, and role fit rather than interpolating document titles or tags. The click endpoint hashes its

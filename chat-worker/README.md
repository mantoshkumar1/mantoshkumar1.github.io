...
Run `npm test`; it executes the unit/contract tests and the 58-case labelled offline evaluation. Run only that evaluation with `npm run evaluate`. The committed evidence and methodology are documented in [`docs/EVALUATION.md`](docs/EVALUATION.md). Test the deployed Worker with:

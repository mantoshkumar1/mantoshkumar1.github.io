...
Run `npm test`; it executes the unit/contract tests and the 553 labelled-case offline evaluation: 73 focused regressions plus 480 recruiter, student, curious-visitor, spouse, colleague, and founder questions. The current result contains 9,678 objective assertions. Run only that evaluation with `npm run evaluate`. The committed evidence and methodology are documented in [`docs/EVALUATION.md`](docs/EVALUATION.md). Test the deployed Worker with:

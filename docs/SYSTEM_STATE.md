...
Last verified Worker deployment: `074a03ea-5030-4998-ad70-c85cd299bf1b`. The active deployed answer-policy cache namespace is `visitor-intent-v35-hiring-journey`.
...
- Page density is intentional: homepage sections render at their real height without synthetic off-screen placeholders; landing, case-study, article, and mobile shells use separate compact spacing scales so the next useful decision arrives sooner without reducing touch-target size. Every project case study opens with the engineering question it answers, then separates the problem, challenge, personal contribution, decisions, before-and-after outcome, available evidence, transferable capabilities, and honest reflection. A four-link closing path connects a related project, Insight, engineering principle, and Ask Mantosh. Empty evidence placeholders are prohibited; confidential projects state their public evidence boundary instead of implying unpublished proof.
...
The chat UI streams the response, immediately renders links, lists, headings, and bold emphasis with a safe built-in Markdown fallback, then sanitizes enhanced Markdown, presents only canonical sources actually used, and turns backend recommendations into two to four related-reading links. It accepts only concise one-line follow-up questions and replaces malformed or overlong suggestions with bounded contextual fallbacks, then shows one compact next-question action above the composer after every successful answer and restored session. New conversations receive a randomized portfolio-wide prompt set, while documented project pages receive three page-specific prompts. The first message submits immediately with a balanced presentation; question intent controls depth and emphasis without classifying the visitor. The UI supports retry/copy actions, traps focus, minimizes without losing the session, restores response metadata with the conversation, exports the visible conversation as a local text file without internal identifiers, and provides an explicit close-and-clear action. Simple greetings, thanks, farewells, capability questions, and bounded light banter receive deterministic conversational replies without retrieval or AI use. Exact public-profile questions read allowlisted facts synchronized from the canonical `about-mantosh` knowledge document into D1; employer, role, location, work-authorization, experience, capability, and skill values are not duplicated in Worker routes. Natural profile wording—including questions about what kind of person or engineer Mantosh is—routes to the published professional profile and technical evidence without inferring private personality. Subjective praise or skepticism is answered as opinion followed by concise published evidence. Home, Projects, Insights, Experience, Resume, Contact, Newsletter, Accessibility, named-project, and latest-article requests navigate directly without retrieval or generation. Before embeddings, a confidence-based D1 lexical gate rejects only sufficiently specific zero-match questions; lexical matches and uncertain questions continue through hybrid retrieval. Allowlisted aggregate counters record gate outcomes, coverage bands, downstream knowledge gaps, and estimated avoided AI calls without visitor text. Unsupported topics receive a helpful scope boundary instead of implying that Mantosh personally has not written about them. If the model omits its Sources section, the Worker inserts the canonical retrieved source rather than exposing an internal citation-formatting failure.
...
- The widget stays uncluttered during normal use and shows a concise retry message only when an AI quota is reached.
